...
from DP_load_dataset_states import read_data
...
def run_dp(method, model, dataset, prn = True, seed = 123, trial = False, select_round = False, **kwargs):
    ...
    # set up the dataset
    Z , num_features, info = read_data(dataset)
...
def sim_dp(method, model, dataset, num_sim = 5, seed = 0, resources_per_trial = {'cpu':4}, **kwargs):
    ...
    if method == 'fedavg':
        ...
        print('--------------------------------Hyperparameter selection--------------------------------')
        print('--------------------------------Seed:' + str(seed) + '--------------------------------')
        config = {'lr': tune.grid_search([.001, .01]),
                'alpha': tune.grid_search([.001])}
        ...
        def trainable(config): 
            return run_dp(method = method, model = model, dataset = dataset, prn = False, trial = True, seed = seed, learning_rate = config['lr'], alpha = config['alpha'], outer_rounds = config['rounds'], inner_epochs = 100//config['rounds'], **kwargs)
