...
class FlowerClientDisparity(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config, average_probabilities=None):
        weight_dict = None
        weights = None
        if os.path.exists(f"{self.fed_dir}/weights_dicts.pkl"):
            with open(f"{self.fed_dir}/weights_dicts.pkl", "rb") as file:
                weight_dict = dill.load(file)
        ...
        if not weight_dict:
            _, weight_dict = Utils.compute_weights(
                targets=np.array(train_loader.dataset.targets),
                sensitive_attributes=np.array(train_loader.dataset.sensitive_features)
                if self.train_parameters.sensitive_attribute == "SEX"
                else np.array(train_loader.dataset.sensitive_features_2),
                weights=np.ones(
                    (np.array(train_loader.dataset.sensitive_features).shape[0], 1)
                ),
                favorable_label=self.train_parameters.favorable_label,
                unfavorable_label=self.train_parameters.unfavorable_label,
                privileged_group=self.train_parameters.priv_group,  # privileged_groups[0],
                unprivileged_group=self.train_parameters.unpriv_group,
            )
        print("computed weights: ", weight_dict)

        if weights is not None:
            train_loader.dataset.weights = torch.tensor(weights)
        else:
            weights = []
            for y, z in zip(
                train_loader.dataset.targets, train_loader.dataset.sensitive_features
            ):
                w = weight_dict[f"(Y={y}, Z={z})"]
                weights.append(w)
            train_loader.dataset.weights = torch.tensor(weights)
