...
class Learning:
    ...
    def train_private_model(
        ...
        with BatchMemoryManager(
            ...
            for batch_number, (data, sens_1, sens_2, target, weigth) in enumerate(
                ...
                if new_weight_dict:
                    new_weights = []
                    for t, sa in zip(target, sensitive_feature):
                        new_weight = new_weight_dict[f"(Y={t}, Z={sa})"]
                        new_weights.append(new_weight)
                    w = torch.tensor(new_weights).to(train_parameters.device)
                else:
                    w = w.to(train_parameters.device)
                ...
                if train_parameters.debias:
                    classic_loss = (criterion(outputs, target) * w).mean()
                else:
                    classic_loss = criterion(outputs, target).mean()
    ...
    def test(
        ...
        with torch.no_grad():
            for data, color, _, target, _ in test_loader:
                target = target.long()
                data, target = (
    ...
    def test_2(
        ...
        with torch.no_grad():
            for data, _, color, target, _ in test_loader:
                target = target.long()
                data, target = (
                    data.to(train_parameters.device),
                    target.to(train_parameters.device),
    ...
    def test_prediction(
        ...
        with torch.no_grad():
            for data, sensitive_attribute, second_sensitive, target, _ in test_loader:
                target = target.long()
                data, target = (
                    data.to(train_parameters.device),
                    target.to(train_parameters.device),
