...
def prepare_tabular_data(
    ...
    if (
            ...
            if cross_silo:
                if sweep:
                    ...
                    custom_dataset = TabularDataset(
                        ...
                        weights=np.ones((X_val.shape[0], 1)),
                    )
                    torch.save(
                        custom_dataset,
                ...
                if train_parameters.sensitive_attribute == "SEX":
                    train_sensitive_attributes.append(Z_train)
                else:
                    train_sensitive_attributes.append(W_train)
                ...
                custom_dataset = TabularDataset(
                    ...
                    weights=np.ones((X_test.shape[0], 1)),
                )
                torch.save(
                    custom_dataset,
        ...
        train_sensitive_attributes = np.concatenate(train_sensitive_attributes)
        train_targets = np.concatenate(train_targets)

        weights_list, weights_dict = Utils.compute_weights(
            targets=train_targets,
            sensitive_attributes=train_sensitive_attributes,
            weights=np.ones((train_sensitive_attributes.shape[0], 1)),
            favorable_label=train_parameters.favorable_label,
            unfavorable_label=train_parameters.unfavorable_label,
            privileged_group=train_parameters.priv_group,  # privileged_groups[0],
            unprivileged_group=train_parameters.unpriv_group,
        )

        all_disparities, max_disparity = Utils.compute_max_disparity_all(
            train_targets,
            train_sensitive_attributes,
            weights_dict,
        )
        print("All disparities: ", all_disparities, " Max disparity: ", max_disparity)

        return fed_dir, None, weights_dict
    else:
