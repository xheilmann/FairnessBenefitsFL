...
class Utils:
    @staticmethod
    def compute_max_disparity_all(y, z, w: dict = None):
        all_disparities = []
        max_disparity = 0
        for target in [0, 1]:
            for sv in [0, 1]:
                disp = RegularizationLoss().compute_violation_with_argmax(
        ...
        return all_disparities, max_disparity
    ...
    def compute_weights(
        ...
        n = np.sum(weights, dtype=np.float64)
        # Counter of samples with the sensitive attribute equal to 1
        n_p = np.sum(
            weights[sensitive_attributes == privileged_group], dtype=np.float64
        )
        # Counter of samples with the sensitive attribute equal to 0
        n_up = np.sum(
            weights[sensitive_attributes == unprivileged_group], dtype=np.float64
        )
        # Counter of samples with target equal to 1
        n_fav = np.sum(weights[targets == favorable_label], dtype=np.float64)
        # Counter of the samples with target equal to 0
        n_unfav = np.sum(weights[targets == unfavorable_label], dtype=np.float64)

        # Counter of samples with target equal to 1 and sensitive attribute equal to 1
        n_p_fav = np.sum(
            weights[
                np.logical_and(
                    targets == favorable_label, sensitive_attributes == privileged_group
                )
            ],
            dtype=np.float64,
        )
        # Counter of samples with target equal to 0 and sensitive attribute equal to 1
        n_p_unfav = np.sum(
            weights[
                np.logical_and(
                    targets == unfavorable_label,
                    sensitive_attributes == privileged_group,
                )
            ],
            dtype=np.float64,
        )
        # Counter of samples with target equal to 1 and sensitive attribute equal to 0
        n_up_fav = np.sum(
            weights[
                np.logical_and(
                    targets == favorable_label,
                    sensitive_attributes == unprivileged_group,
                )
            ],
            dtype=np.float64,
        )
        # Counter of samples with target equal to 0 and sensitive attribute equal to 0
        n_up_unfav = np.sum(
            weights[
                np.logical_and(
                    targets == unfavorable_label,
        ...
        weights[
            np.logical_and(
                targets == privileged_group, sensitive_attributes == favorable_label
            )
        ] = (
            weights[
                np.logical_and(
        ...
        weights[
            np.logical_and(
                targets == unprivileged_group, sensitive_attributes == favorable_label
            )
        ] = (
            weights[
                np.logical_and(
                    targets == unprivileged_group,
                    sensitive_attributes == favorable_label,
                )
            ]
            * w_p_unfav
        )
        weights[
            np.logical_and(
                targets == privileged_group, sensitive_attributes == unfavorable_label
            )
        ] = (
            weights[
                np.logical_and(
                    targets == privileged_group,
                    sensitive_attributes == unfavorable_label,
                )
            ]
            * w_up_fav
        )
        weights[
            np.logical_and(
                targets == unprivileged_group, sensitive_attributes == unfavorable_label
            )
        ] = (
            weights[
                np.logical_and(
        ...
        weight_dict = {
            f"(Y={favorable_label}, Z={privileged_group})": w_p_fav,
            f"(Y={favorable_label}, Z={unprivileged_group})": w_p_unfav,
            f"(Y={unfavorable_label}, Z={privileged_group})": w_up_fav,
            f"(Y={unfavorable_label}, Z={unprivileged_group})": w_up_unfav,
        }
